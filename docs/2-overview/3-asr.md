...
### Why
Anticipatory Story Reader (ASR) gets its name from a machine reading process which simulates that of a young child learning to read, a process of growing and maintaining a "world model" through reading and personal experiences such as visiting the zoom.
...
Those goals entail important features, such as statistics for later Bayesian modeling, aid in disambituation, and others.
...
#### WordGram Graph
Actually, it's a TermGram Graph since each WordGram object can represent and give identity to from one to many word phrases, both nouns and verbs. By capturing all novel (new to the system words or pharses) in individual database objects and giving them a numeric identity, we create symbols, and we compress large quantities of text into an space which grows slower over time.

Each WordGram object contains, among other things:
- The words
- Numeric identity
- Part of Speech (POS) - note: some terms can take different POS values under different contexts
- Predicate status: positive, negative, speculative
- Array of sentence identies in which the words are captured, the cardinality of which supports statistical analysis
- Array of TopicMap topic identities in which the words provide a name (label) or a synonym - note: ambiguity can arise when a term is a label for more than one topic
- DBpedia URI - if any
- WikiData URI - if any
#### nlp-world-model
...
- Dictionary handling is primary to the ability to maintain the WordGram Graph; typographical errors may creep into the system. The Dictionary supports finding an identity when given text, or finding text when given an identity.  If text is provided and not found, an identifier is created and returned, along with a flag which says "this is new", which triggers WordGram creation
- WordGram creation includes taking all sources from the parser, Dictionary, and creating a new WordGram.  A background process then looks for that text in DBpedia and Wikidata.
### Glossary
#### Ambiguity
